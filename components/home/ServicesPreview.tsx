...
export default function ServicesSection() {
  return (
    <section className="bg-[#f8f9fc] py-16 px-4 sm:px-6 lg:px-8 text-slate-800 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES_DATA.map((service, index) => {
            // Determine animation initial state based on 3-column index
            const col = index % 3;
            let initialProps = { opacity: 0, x: 0, y: 0 };

            if (col === 0) {
              // Left column -> Slide from left
              initialProps = { opacity: 0, x: -60, y: 0 };
            } else if (col === 1) {
              // Middle column -> Slide up from bottom
              initialProps = { opacity: 0, x: 0, y: 50 };
            } else {
            ...
            return (
              <motion.div
                ...
                transition={{
                  duration: 0.8,
                  ease: "easeOut",
                  delay: (index % 3) * 0.15, // Staggered delay across the row
                }}
